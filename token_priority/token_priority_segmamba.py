...
class MambaLayer(nn.Module):
    def __init__(self, dim, d_state=16, d_conv=4, expand=2, num_slices=None):
        ...
        self.soft_sort = RefSliceSoftSort(m=4096, tau=1.0, hard=True)  # 새 클래스
    ...
    def forward(self, x):
        ...
        """
        # ───────── 수정 후 ─────────
        perm = self.soft_sort(scores)  # (B, n_tokens)  전역 인덱스, 수정
        # gather 로 재배열(수정)
        x_sorted = torch.gather(
            x_norm, 1, perm.unsqueeze(-1).expand(-1, -1, self.dim)
        )
