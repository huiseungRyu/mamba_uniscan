...
"""
def semantic_neighbor(x, index):
    # x: (B, N, hidden), index: (B, N), 둘 다 GPU 상이라고 가정
    # 1) 한 번만 unsqueeze & expand
    idx = index.unsqueeze(-1).expand(-1, -1, x.size(-1))  # (B, N, hidden)
    # 2) 단 한번의 gather
...
def add_heading_avg(seq):
    """시퀀스 맨 앞에 평균 토큰 삽입: (B,N,C)→(B,N+1,C)"""
    avg = seq.mean(1, keepdim=True)
...
class ASSM(nn.Module):
    def __init__(self, dim, d_state, input_resolution, num_tokens=2048, inner_rank=128, mlp_ratio=2., idx=0):
        super().__init__()
        self.dim = dim
        ...
        self.idx = idx # causal 방식 도입하며 추가
        ...
        self.CPE3d = nn.Conv3d(hidden, hidden, kernel_size=3, stride=1,
    ...
    def forward(self, x, x_size, token, **kwargs):
        ...
        """
        x_sort_values, x_sort_indices = torch.sort(detached_index, dim=-1)

        # 2) GPU로 옮기기
        x_sort_values = x_sort_values.to(x.device, non_blocking=True)
        x_sort_indices = x_sort_indices.to(x.device, non_blocking=True)
        """

        x_sort_values, x_sort_indices = detached_index.sort(dim=-1)

        # 3) reverse index
        x_sort_indices_reverse = x_sort_indices.argsort(dim=-1)
        ...
        hidden = x_3d.shape[1]
        """
        if 'global_token' in kwargs:
            g_embed = kwargs['global_token'].view(B, C, 1, 1, 1)
        else:
            # 평균을 embed_dim 기준으로 직접 계산
            g_embed = x_3d.mean(dim=[2, 3, 4], keepdim=True)  # (B, hidden,1,1,1)
            g_embed = g_embed.detach()  # (B, hidden,1,1,1)
            g_embed = x.permute(0, 2, 1).mean(-1, keepdim=True)  # (기존 평균 코드 제거)
            g_embed = g_embed.view(B, C, 1, 1, 1)
        """
        # embed-dim → hidden-dim 으로 투영 (Conv3d 재사용)
        #g_proj = self.in_proj3d(g_embed)                          # (B, hidden, 1,1,1)
        #g_proj = g_proj.flatten(2).transpose(1, 2)                # (B, 1, hidden)

        x_flat = x_3d.flatten(2).transpose(1, 2).contiguous()     # (B, n, hidden)
        g_proj = x_flat.mean(dim=1, keepdim=True)  # (B, 1, hidden)
        semantic_x = semantic_neighbor(x_flat, x_sort_indices)    # 토큰 재배치 완료

        flip = (self.idx % 2 == 1)  # 홀수 블록만 뒤집기
        if flip:
            semantic_x = flip_tokens(semantic_x)

        semantic_x = torch.cat([g_proj, semantic_x], dim=1)        # (B, n+1, hidden)

        # prompt 길이 맞추기 (앞에 0-vector 하나)
        pad = torch.zeros((B, 1, prompt.size(2)), device=prompt.device, dtype=prompt.dtype)
        prompt = torch.cat([pad, prompt], dim=1)  # (B, n+1, d_state)
    ...
    def forward(self, x, x_size, token, **kwargs):
        ...
        prompt = semantic_neighbor(prompt, x_sort_indices)  # ★ 추가 ❶

        # ---------- 3. Adventurer-style 전역 토큰 + 플리핑 -------- #
        if 'global_token' in kwargs:
            # embed_dim 차원의 토큰
            g_embed = kwargs['global_token']              # [B, 1, C]
            # 공간 맵으로 변환 & hidden 투영
            g_map   = g_embed.squeeze(1).view(B, C, 1, 1).expand(B, C, H, W)
            g_hidden_map = self.in_proj(g_map)            # -> [B, hidden, H, W]
            g_hidden_map = g_hidden_map * torch.sigmoid(self.CPE(g_hidden_map))
            # flatten & 평균 내기 → [B, 1, hidden]
            g_flat = g_hidden_map.flatten(2).transpose(1,2)
            g = g_flat.mean(dim=1, keepdim=True)
        else:
            # fallback: 로컬 semantic_x 평균
            g = semantic_x.mean(dim=1, keepdim=True)      # [B, 1, hidden]

        flip = (self.idx % 2 == 1)  # 홀수 블록만 뒤집기
        if flip:
            semantic_x = flip_tokens(semantic_x)
            prompt = flip_tokens(prompt)  # ★ 추가 ❷

        semantic_x = torch.cat([g, semantic_x], dim=1)            # (B, N+1, hidden)
        # 1) prompt 차원에 맞는 0 패딩 생성
        pad = torch.zeros((prompt.size(0), 1, prompt.size(2)),
                          device=prompt.device, dtype=prompt.dtype)
        # 2) prepend
        prompt = torch.cat([pad, prompt], dim=1)
...
class ASSB(nn.Module):
    def __init__(self,
        ...
        self.patch_embed3d   = PatchEmbed3D(patch_size=patch_size,
                                          in_chans=dim,
                                          embed_dim=dim,
                                          norm_layer=None)
        self.patch_unembed3d = PatchUnEmbed3D(patch_size=patch_size,
                                            in_chans=dim,
                                            embed_dim=dim,
                                            norm_layer=None)
    ...
    def forward(self, x, x_size, params=None):
        # x: (B, N, C), x_size=(D', H', W')
        # residual_group 내부에서 ASSM → ConvFFN(=dwconv3d) 수행
        x_rg = self.residual_group(x, x_size, params)

        # unembed → conv3d → embed
        feat = self.patch_unembed(x_rg, x_size)  # (B,C,D,H,W)
        feat = self.conv(feat)                   # (B,C,D,H,W)
        out  = self.patch_embed(feat)            # (B,N,C)
        return out + x
...
class PatchEmbed3D(nn.Module):
    """(B, C, D, H, W) -> (B, N, C) 토큰 시퀀스 (Conv3d 기반)"""
    def __init__(self,
                 patch_size: int or tuple = 1,
                 in_chans: int = 48,
                 embed_dim: int = 48,
                 norm_layer=None):
        super().__init__()
        patch_size = to_3tuple(patch_size)
        # non-overlapping 3D 패치 추출
...
class PatchUnEmbed3D(nn.Module):
    """(B, N, C) -> (B, C, D, H, W) 복원 (ConvTranspose3d 기반)"""
    def __init__(self,
                 patch_size: int or tuple = 1,
                 in_chans: int = 48,
                 embed_dim: int = 48,
        ...
        patch_size = to_3tuple(patch_size)
        # 역 Conv3d로 패치 병합
        self.unproj = nn.ConvTranspose3d(embed_dim,
                                         in_chans,
                                         kernel_size=patch_size,
                                         stride=patch_size)
        self.norm = norm_layer(in_chans) if norm_layer else None
    ...
    def forward(self, x, x_size):
        ...
        Dp, Hp, Wp = x_size
        # (B, C, D', H', W')
        x = x.transpose(1,2).view(B, C, Dp, Hp, Wp)
        # -> (B, in_chans, D, H, W)
        x = self.unproj(x)
        return self.norm(x) if self.norm else x
